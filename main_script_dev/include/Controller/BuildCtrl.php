...
class BuildCtrl extends GameCtrl
{
    ...
    public function __construct()
    {
        ...
        $this->view->vars['bodyCssClass'] = $this->selectedBuildingIndex <= 18 || $this->selectedBuildingIndex == 39 ? "build perspectiveResources" : "build perspectiveBuildings";
        $this->view->vars['contentCssClass'] = $this->selectedBuildingIndex <= 18 ? "contentPage" : "contentPage";
		...
		$this->view->vars['answerId'] = $this->getAnswerIdForBuilding($field['item_id']);
        ...
        $this->buildView->vars['titleInHeader'] = $this->view->vars['titleInHeader'];
	...
	private function getAnswerIdForBuilding($gid){
		switch($gid){
			case 0: return 26;
			case 1: return 195;
			case 2: return 189;
    ...
    private function getBuildingWrapper($item_id, $build, $soon = FALSE)
    {
		...
		$data['race'] = $this->session->getRace();
		$data['raceName'] = $this->session->getRaceName();
    ...
    private function getActionText($item_id = 0)
    {
        ...
        if (!$session->hasPlus() && $workerResult['isBusy']) {
            $result['plus'] = getButton(["type" => "button", "class" => "textButtonV1 gold builder", "value" => T("Buildings", "buildingQueue.name"),], ["data" => ['type' => 'button', "plusDialog" => ["featureKey" => 'buildingQueue', 'infoIcon' => 'http:\/\/t4.answers.travian.com\/index.php?aid=Travian Answers#go2answer',],],], T("Buildings", "buildingQueue.name"));
            $result['plus'] .= '<br>';
        ...
        $btn = getButton(["type" => "button", "class" => "textButtonV1 green " . ($new ? 'new' : 'build'), "onclick" => "window.location.href = '$link'; return false;",], ['data' => ["class" => "textButtonV1 green " . ($new ? 'new' : 'build')]], $lvl == 1 && $item_id > 4 ? T("Buildings", "constructBuilding") : sprintf(T("Buildings", "upgradeBuilding"), $lvl));
        $result['waitLoop'] = $workerResult['isPlusUsed'];
        if ($workerResult['isBusy']) {
            $result['main'] = '<span class="errorMessage">' . T("Buildings", "workersBusy") . '</span><br>';
            //$result['main'] = null;
            goto outReturn;
        } else {
            $result['main'] = $btn;
    ...
    public function getBuildingContract()
    {
		...
		$contract['raceName'] = $this->session->getRaceName();
