...
class HeroAdventureCtrl extends GameCtrl
{
    ...
    private function renderAdventure($row)
    {
        ...
        $HTML .= '<td class="place" style="background-image: url(../../gpack_dev/29c89d54/mainPage/img_ltr/scenes/HeroV2/place/grassland_small.jpg)">';
        $wdata = $db->query("SELECT fieldtype, oasistype, landscape, occupied FROM wdata WHERE id={$row['id']}")->fetch_assoc();
        ...
        //     } else {
               ...
               if($name==null)
               {
                $name=T("HeroAdventure","grassland");
               }
        ...
        $HTML .= '<img alt="' . T("HeroAdventure", $name) . '" title="' . T("HeroAdventure", $name) . '" class="grassland" src="img/x.gif">';
        // $HTML .= $name;
        ...
        $HTML .= '<button class="textButtonV2 buttonFramed rectangle withText green " id="explore" type="button" data-mapid="'.T("HeroAdventure", $row['kid']). '"><div><div>Explore</div></div></button>';
