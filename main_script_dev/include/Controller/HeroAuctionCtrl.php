...
$TempArray1 = $db->query("UPDATE auction SET  finish=0 WHERE uid='$queryVariable' AND type='$getId' ");
$TempArray=$db->query("UPDATE items SET  proc=1 WHERE uid='$queryVariable' AND type='$getId' ");
$sendId=$_POST['sendId'];

$TempArray2 = $db->query("UPDATE auction SET  finish=1 WHERE uid='$queryVariable' AND type='$sendId' ");
$TempArray3=$db->query("UPDATE items SET  proc=0 WHERE uid='$queryVariable' AND type='$sendId' ");
class HeroAuctionCtrl extends GameCtrl
{
    ...
    private function buy($canTakePlaceInAuction)
    {
        ...
        $total = $m->getTotalBuyAuction(0,
            $filter);
        $result = $m->getBuyAuction(0,
            $page,
            20,
            $filter);
    ...
    private function renderAuction($canTakePlaceInAuction, $row, $current_silver, $page, $filter, $isEdit = FALSE)
    {
        ...
        if ($this->spreadRandom) {
            $template .= '<a class="bidButton openedClosedSwitch switchClosed"><span class="errorMessage">' . ($isEdit ? T("Auction",
                    "doBid") : T("Auction", "bid")) . '</span></a>';
        } else if ($current_silver >= $silver || $row['activeUid'] == $session->getPlayerId()) {
            if (!$canTakePlaceInAuction) {
                $template .= '<a class="bidButton openedClosedSwitch switchClosed" title="' . T("Auction",
    ...
    private function sell($canTakePlaceInAuction)
    {
        ...
        if (!$canTakePlaceInAuction) {
            ...
            $this->view->vars['content'] .= '<div class="clear"></div>';
            $this->view->vars['content'] .= '<div class="bar" style="width:' . ($session->getSuccessAdventuresCount() * 10) . '%;"></div></div>';
        }
        $maxAuctions = 5;
        $m = new AuctionModel();
        $db = DB::getInstance();
        ...
        if ($running->num_rows == 0) {
            ...
            while ($row = $running->fetch_assoc()) {
                ...
                if ($row['bids'] == 0) {
                    $view->vars['auctions'] .= '<button class="btnbids" id="btn'. $row['type'] .' "><img alt="delete" src="img/x.gif" class="del"></button>';
                }
