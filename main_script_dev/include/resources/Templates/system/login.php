<script type="text/javascript">
    $.fn.showOrHide = function (imgid) {
			if (imgid != '') {
				if ($(imgid).hasClass('close')) {
						$(imgid).addClass('open');
						$(imgid).removeClass('close');
				} else if ($(imgid).hasClass('open')) {
						$(imgid).addClass('close');
						$(imgid).removeClass('open');
				}
			}
            this.toggleClass('hide');
        };
</script>
<h1 class="titleInHeader"><?=T("Login", "Login"); ?></h1>
