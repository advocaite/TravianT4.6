            ...
            <div class="paperTop">
                <div class="paperContent">
                    ...
                    <div id="recipient">
                        ...
                        <div class="header text">
                            ...
                            <button type="button" id="adbook" class="icon " title="<?= T("Messages", "Addressbook"); ?>"
                                    onclick="Travian.Game.AddressBook.show();" tabindex="5">
                    ...
                    <div id="send">
                        ...
                        <button type="submit" value="send" name="s1" id="s1" class="textButtonV1 green " tabindex="4" onclick="return MessageSendButtonDoubleClickPreventer.check();" version="textButtonV1"><?= T("Messages", "send"); ?></button>
						<script type="text/javascript">
                            jQuery(function () {
                                if (jQuery('#s1')) {
...
<script type="text/javascript" data-cmp-info="6">
	...
	jQuery(function() {
        ...
        new Travian.Game.AutoCompleter.PlayerName(jQuery('input#receiver'), {'context': 'messageRecipient'});

        var receiver = jQuery('#receiver');
        var supportNotAvailableViaIGMAnymore = jQuery('#supportNotAvailableViaIGMAnymore');
        var bbEditor = jQuery('#bbEditor');
        var sendButton = jQuery('#send').find('button');

        var ajaxReq = null;

        receiver.on('change keyup', function() {
            var recipients = Travian.Game.Messages.getRecipients(true);

            if (T4_feature_flags.multi_language) {
                if (ajaxReq) clearTimeout(ajaxReq);
                ajaxReq = setTimeout(() => {
                    ajaxReq = null;
                    Travian.api('profile-languages/recipients', {
                        data: {
                            recipients: recipients,
                            sender: 6109                        },
                        success: function (response) {
                            jQuery('#languages #playerLanguages').html(response.languages);
                            Travian.Tip.refresh();
                        }
                    })
                }, 333);
			...
			<?php /*
            var isInputHidden = bbEditor.hasClass('hide');
            var shouldInputBeHidden = Travian.Game.Messages.checkIfRecipientsContainTechnicalAccounts(recipients);
            if (isInputHidden !== shouldInputBeHidden) {
                supportNotAvailableViaIGMAnymore.toggleClass('hide');
                bbEditor.toggleClass('hide');
