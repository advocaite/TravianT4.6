...
<div class=" infoArea">
        ...
        function buildFeature($featureName, $coins, $wwInAvailable, $delivery, $img='')
        {
            ...
            $HTML .= '</div>';
			$HTML .= '<div class="featureButton">' . (new GoldHelper())->renderBuyButton($featureName, $coins, $wwInAvailable);
        ...
        function buildBuildings($featureName,$title, $coins, $wwInAvailable, $delivery, $bid)
        {
            ...
            if(substr($bid, 0, 3) != 'res'){
                ...
                $HTML .= '<div class="premiumFeatureBuilding" style="width:auto;height:100%"><img src="img/x.gif" class="building iso '.Session::getInstance()->getRaceName().' g'.$bid.'" alt=""></div>';
            }else{
                $disabled = true;
            ...
            $HTML .= '<div class="featureRemainingTime featureSubtitle subtitle"><span class="">' . T("PaymentWizard", $featureNameDescForTranslation) . '</span></div>';
            $HTML .= '</div>';
			$HTML .= '<div class="featureButton">' . (new GoldHelper())->renderBuyButton($featureName, $coins, $wwInAvailable,$disabled);
            $HTML .= '<div class="featureDuration featureRenewal featureButtonSubtitle subtitle">' . T("PaymentWizard", "delivery") . ': <span class="bold">' . ($delivery == 0 ? T("PaymentWizard", "Immediately") : $delivery) . '</span>  ' . ($delivery == 0 ? '' : T("PaymentWizard", "Minutes")) . '  </div>';
            $HTML .= '</div>';
            $HTML .= '</div>';
            $HTML .= '</div>';
        ...
        function buildAnimalFeature($featureName, $modifier, $coins, $wwInAvailable, $delivery)
        {
            ...
            $HTML .= '<div class="featureContent">';
			$HTML .= '<div class="p2">';
            $HTML .= '<table><tbody><tr style="border: 0;">';
        ...
        {            
            ...
            foreach ($packs as $index => $package) {
				...
				$resourceName ="";
				switch($resourceId){
					case 1: $resourceName="lumber";break;
					case 2: $resourceName="clay";break;
					case 3: $resourceName="iron";break;
					case 4: $resourceName="crop";break;
				}
                ...
                $HTML .= '<div class="featureData single">';
                    ...
                    $HTML .= '<i class="'.($resourceName).'_huge"></i>';   
                    $HTML .= '</div>';    
                    $HTML .= '<div class="featureAmt">';
                        $HTML .= '<div class="inlineIcon resources r'.($resourceId).'">&nbsp;<span class="value ">' . number_format_x($resourceAmt) . '</span></div>';
                    $HTML .= '</div>';     
                ...
                $HTML .= '<div class="featureData combo">';
                    ...
                    $HTML .= '<div class="featureImg">';       
                      for($i = 0; $i < 4; $i++){                            
                            ...
                            $HTML .= '<div class="inlineIcon resources"><i class="r'.($resourceId).'Big"></i></div>';
                        }
                    $HTML .= '</div>';    
                    $HTML .= '<div class="featureAmt resourcesInline">';
                        ...
                        for($i = 0; $i < 4; $i++){                            
                            $resourceId = $i+1;
                            $resourceAmt =  ($package['hours']) * $productionAvg;
                            $HTML .= '<div class="inlineIcon resources"><span class="value ">&nbsp;' . number_format_x($resourceAmt) . '</span></div>';
                        }
                    $HTML .= '</div>';     
                    $HTML .= '<div class="featureBuyButton">';     
                        $HTML .= '<div class="">' . (new GoldHelper())->renderBuyButton($featureName, $coins, $wwInAvailable) . '</div>';
...
<div class=" contentArea">
        <div class="paymentPopupDialogWrapper scrollableContent ">
            <?php if ($vars['enabledFeatures']['general']):$step++; ?>
                <div
