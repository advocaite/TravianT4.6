...
class GameView
{
    ...
    public function __construct()
    {
        ...
        $this->vars['race'] = $this->session->getRace();
        $this->vars['raceName'] = $this->session->getRaceName();
        $this->vars['titleInHeader'] = '';
        $this->vars['bodyCssClass'] = '';
        $this->vars['contentCssClass'] = '';
        $this->vars['containerCssClass'] = 'contentPage';
    ...
    private function renderHeroBox()
    {
        ...
        $db = DB::getInstance();
        $view = new PHPBatchView("layout/topBarBoxHero");
        ...
        $beard=$db->fetchScalar("SELECT beard FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $headProfile=$db->fetchScalar("SELECT headProfile FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $hairColor=$db->fetchScalar("SELECT hairColor FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $hairStyle=$db->fetchScalar("SELECT hairStyle FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $ears=$db->fetchScalar("SELECT ears FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $eyebrow=$db->fetchScalar("SELECT eyebrow FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $eyes=$db->fetchScalar("SELECT eyes FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $nose=$db->fetchScalar("SELECT nose FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
        $mouth=$db->fetchScalar("SELECT mouth FROM face WHERE uid=" . Session::getInstance()->getPlayerId());
...
//                 "raceName"             => $this->session->getRaceName(),
...
//         }
    ...
    private function renderInfoBox()
    {
        ...
        $view->vars['unread'] = $unread ? "New" : 'Read';
        //$view->vars['content'] = '';
        ...
        if ($infoBox_count == 0) {
			$view->vars['total'] = 0;
			$view->vars['content'] .= '<li id="infoID_0" class="infoType_1 firstElement active unreaded"><span style="font-weight:bold;">No new messages!</span><br></li>';
			//return;
		}
        $output = $view->output();
    ...
    function renderActiveVillageBox()
    {
        ...
        if (true || !($data = $cache->get("ActiveVillageBox:{$kid}"))) {
            ...
            $data['workshop'] = getAButton([
                'id'      => $data['workshopButtonId'] = get_button_id(),
                "type"    => "button",
                "class"   => "layoutButton buttonFramed withIcon round workshop " . ($allowed ? 'green' : "gold") . (Village::getInstance()->hasWorkshop() ? '' : ' disabled'),
                "title"   => htmlspecialchars($title),
				"svg"	  => "workshop_". ($allowed ? 'green' : "gold"),
                "href"  => ($allowed ? (Village::getInstance()->hasWorkshop() ? 'build.php?id=' . $data['workshopBuildingFieldId'] : '') : '#'),
            ],
                ["data" => $d]);
            ...
            $data['stable'] = getAButton([
                'id'      => $data['stableButtonId'] = get_button_id(),
                "type"    => "button",
                "class"   => "layoutButton buttonFramed withIcon round stable " . ($allowed ? 'green' : "gold") . (Village::getInstance()->hasStable() ? '' : ' disabled'),
                "title"   => htmlspecialchars($title),
				"svg"	  => "stable_". ($allowed ? 'green' : "gold"),
                "href"  => ($allowed ? (Village::getInstance()->hasStable() ? 'build.php?id=' . $data['stableBuildingFieldId'] : '') : '#'),
            ],
                ["data" => $d]);
            ...
            $data['barracks'] = getAButton([
                'id'      => $data['barracksButtonId'] = get_button_id(),
                "type"    => "button",
                "class"   => "layoutButton buttonFramed withIcon round barracks " . ($allowed ? 'green' : "gold") . (Village::getInstance()->hasBarracks() ? '' : ' disabled'),
                "title"   => htmlspecialchars($title),
				"svg"	  => "barracks_". ($allowed ? 'green' : "gold"),
                "href"  => ($allowed ? (Village::getInstance()->hasBarracks() ? 'build.php?id=' . $data['barracksBuildingFieldId'] : '') : '#'),
            ],
                ["data" => $d]);
            ...
            $data['marketplace'] = getAButton([
                'id'      => $data['marketplaceButtonId'] = get_button_id(),
                "type"    => "button",
                "class"   => "layoutButton buttonFramed withIcon round market " . ($allowed ? 'green' : "gold") . (Village::getInstance()->hasMarketPlace() ? '' : ' disabled'),
                "title"   => htmlspecialchars($title),
				"svg"	  => "marketplace_". ($allowed ? 'green' : "gold"),
                "href"  => ($allowed ? (Village::getInstance()->hasMarketPlace() ? 'build.php?id=' . $data['marketplaceBuildingFieldId'] : '') : '#'),
            ],
                ["data" => $d]);
            $cache->add("ActiveVillageBox:{$kid}", $data, 60);
        }
        $data['villageName'] = Village::getInstance()->getName();
        $data['villageId'] = Village::getInstance()->getKid();
    ...
    function renderVillagesList()
    {
        ...
        $data['vcount'] = $i;
        $data['total_vcount'] = $count;
        $data['currentCP'] = $currentCP - $currentMaxVillageCP;
        $data['nextCP'] = $dif;
        $session = $this->session;
        $villages = (new ProfileCache($this->session->get("profileCacheVersion")))->getProfileVillages($this->session->getPlayerId());
        $attacksList = [];
        ...
        foreach ($villages as $index=>$row) {
			...
			$data['villages'] .= '<div class="dropContainer" data-sortindex="' . $index .'">';
			$data['villages'] .= 	'<div class="listEntry ' . $active . ($attacks ? ' attack' : '').'" data-did="' . $row['kid'] .'">';
            $data['villages'] .= 		'<div class="dragAndDrop preventMobileSwipeNavigation">';
			$data['villages'] .= 			$svg['village_drag_n_drop'];
			$data['villages'] .= 		'</div>';
			$data['villages'] .= 		'<a href="?newdid=' . $row['kid'] . (!empty($prefix) ? '&' . $prefix : '') . '" class="' . $active.'" title="' . htmlspecialchars($title) . '">';
			$data['villages'] .= 			'<span class="iconAndNameWrapper">';
			$data['villages'] .= 				'<span class="incomingTroops" data-id="' . $row['kid'] .'">';
			$data['villages'] .= 					'<svg viewBox="0 0 20 19.06" class="attack">';
			$data['villages'] .= 						$svg['village_incomming_attack'];
			$data['villages'] .= 					'</svg>';
			$data['villages'] .= 				'</span>';
			$data['villages'] .= 				'<span class="name">' . $name . '</span>';
			$data['villages'] .= 			'</span>';
		    $data['villages'] .= 		'</a>';
			$data['villages'] .= 		'<span class="coordinatesGrid">' . $value . '</span>';
			$data['villages'] .= 	'</div>';
			$data['villages'] .= '</div>';
        }
