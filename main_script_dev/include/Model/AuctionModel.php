...
//            }
    ...
    public function getTotalBuyAuction($uid, $filter)
    {
        ...
        $db = DB::getInstance();
        if ($filter == 0) {
            return $db->fetchScalar("SELECT COUNT(id) FROM auction WHERE finish=1 AND uid!=$uid");
        }
        return $db->fetchScalar("SELECT COUNT(id) FROM auction WHERE finish=1 AND uid!=$uid AND btype=$filter");
    }
    ...
    public function getBuyAuction($uid, $page, $pageSize, $filter)
    {
        ...
        $db = DB::getInstance();
        if ($filter == 0) {
            return $db->query("SELECT * FROM auction WHERE finish=1 AND uid!=$uid ORDER BY time ASC LIMIT " . (($page - 1) * $pageSize) . ", " . $pageSize);
        }
        return $db->query("SELECT * FROM auction WHERE finish=1 AND uid!=$uid AND btype=$filter ORDER BY time LIMIT " . (($page - 1) * $pageSize) . ", " . $pageSize);
    }
