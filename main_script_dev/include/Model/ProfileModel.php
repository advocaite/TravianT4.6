...
class ProfileModel
{
    ...
    public function getPlayerVillages($uid, $byName = FALSE)
    {
        $db = DB::getInstance();
        if ($byName) {
            return $db->query("SELECT kid, seq, name, owner, isWW, pop, capital FROM vdata WHERE owner=$uid ORDER BY name");
        }
        return $db->query("SELECT kid, seq, name, owner, isWW, pop, capital FROM vdata WHERE owner=$uid ORDER BY seq ASC");
    }
